// console/lib/server/credentials.ts
// Credential isolation (threat model §7 / T4b). Ported — NOT imported — from
// web/lib/security/credentials.ts. The server is the SOLE credential holder and the
// browser must NEVER see a credential. Invariants:
//
...
//    for any route/SSE path — OR any spawned child — to accidentally reflect.
...
/**
 * Fail-closed guard for the browser-facing serialization boundary (JSON responses,
 * SSE frames). Throws if the payload references a credential env NAME or contains a
 * known stripped secret VALUE. The error never includes the value.
 */
