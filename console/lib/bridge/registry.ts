// console/lib/bridge/registry.ts
// Server-owned provenance registry for harness args (threat model §7 / T1). Ported —
// NOT imported — from web/lib/daemon/registry.ts.
//
// A lane slug, trace session, or plan file may be passed to harness.sh ONLY if the
// server minted it here — matching a regex is NOT sufficient on its own.
//
// HARD CONTRACT: mint* must be called ONLY with server-generated/derived values (the
// lanes the daemon planned, the session ids it generated, the plan files it wrote).
// NEVER pass client/browser input to mint* — doing so launders untrusted input into
// "provenance" and defeats the whole control. The mint-time regex is a shape check
// (defense in depth), not the trust boundary; provenance is.
//
...
/** Record a lane slug the server planned; only minted slugs reach harness.sh. */
